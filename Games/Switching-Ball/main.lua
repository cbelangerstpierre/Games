...
---@diagnostic disable-next-line: discard-returns
    ...
    file = io.open("Switching-Ball/Highscore.txt", "r")
...
function createRectangle(orientation, color)
    ...
    return Image(
    RECTANGLE_FILE,
    MID_CIRCLE_X+(CIRCLE_RADIUS+2*num-border_adjuster)*
    math.sin(math.rad(orientation))-RECTANGLE_WIDTH/2*math.cos(math.rad(orientation)),
    MID_CIRCLE_Y-(CIRCLE_RADIUS+2*num-border_adjuster)*
    math.cos(math.rad(orientation))-RECTANGLE_WIDTH/2*math.sin(math.rad(orientation)),
    RECTANGLE_WIDTH,
    ...
    if gameState == "play" then
        ...
        if ball:touchRectangle(rectangles) then
            ...
            if score == highscore then
                file = io.open("Switching-Ball/Highscore.txt", "w")
                file:write(score)
                file:close()
            end
        end
